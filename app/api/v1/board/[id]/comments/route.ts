...
import { listBoardComments, createBoardComment } from '@/lib/supabase/repositories/board';

// GET /api/v1/board/[id]/comments — 댓글 목록 (인증된 유저만)
export async function GET(_request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (profile?.role !== 'admin' && profile?.verification_status !== 'approved') {
      return NextResponse.json({ data: null, error: { code: 'FORBIDDEN', message: '인증된 유학생만 댓글을 볼 수 있습니다.' } }, { status: 403 });
    }

    const comments = await listBoardComments(params.id);
    return NextResponse.json({ data: comments, error: null });
  } catch (err) {
    if (process.env.NODE_ENV !== 'production') console.error('[GET /api/v1/board/[id]/comments]', err);
    return NextResponse.json({ data: null, error: { code: 'FETCH_FAILED', message: '댓글을 불러오지 못했습니다.' } }, { status: 500 });
  }
}

// POST /api/v1/board/[id]/comments — 댓글 작성 (인증된 유저만)
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (process.env.NODE_ENV !== 'production') console.error('[POST /api/v1/board/[id]/comments]', err);
