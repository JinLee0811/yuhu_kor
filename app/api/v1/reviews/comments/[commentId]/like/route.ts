...
import { likeReviewCommentDb } from '@/lib/supabase/repositories/reviews';

// POST /api/v1/reviews/comments/[commentId]/like — 댓글 좋아요 (로그인 필요)
export async function POST(_request: NextRequest, { params }: { params: { commentId: string } }) {
  try {
    ...
    await likeReviewCommentDb(params.commentId);
    return NextResponse.json({ data: null, error: null });
  } catch (err) {
    if (process.env.NODE_ENV !== 'production') console.error('[POST /api/v1/reviews/comments/[commentId]/like]', err);
